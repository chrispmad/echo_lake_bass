...
```{r trim_lake_skeleton_pieces_from_streams_layer}
...
# Remove streams that are actually 'lake skeletons' and are completely 
# covered by a lake.
nstreams_to_draw_from_trimmed = map(1:nrow(nstreams), ~ {
  # This stream doesn't overlap with any lake. Don't touch it!
  if(is.na(row_lakes_int_streams[.x])){
    stream_out = nstreams[.x,]
  } else {
    stream_minus_lake = st_difference(nstreams[.x,],
                                      dplyr::select(
                                        nlakes[row_lakes_int_streams[.x],],
                                        geometry)
    )
...
```{r join_freshwater_atlas_layers_together}
wbs = nlakes |> 
  bind_rows(nrivers, nstreams_to_draw_from_trimmed)
...
We need to trace a course from `r focus_wb` through following the direction of flow through contiguous streams and lakes.
...
for(i in 1:30){
  ...
  ggplot() + geom_sf(data = flow_seed, fill = 'darkblue') + 
    geom_sf(data = nearest_wbs, fill = 'lightblue')
  
  # No more wbs that are touching our flow_seed? Break!
  if(nrow(nearest_wbs) == 0){break}
  
  # And remove these nearest waterbodies from our table of waterbodies to draw from...
  wb_pool_to_draw_from = wb_pool_to_draw_from[-unlist(st_intersects(st_buffer(flow_seed,20),wb_pool_to_draw_from)),]
  ...
  # Find the nearest flow markers within 50 meters of our focus wb.
  nearest_flow_markers = flow_lines_to_draw_from[unlist(st_is_within_distance(flow_seed, flow_lines_to_draw_from, dist = 50)),]
  
  ggplot() + 
    geom_sf(data = flow_seed, fill = 'darkblue') + 
    geom_sf(data = wbs_after_difference, fill = 'lightblue') + 
  ...
  flow_lines_to_draw_from = flow_lines_to_draw_from[-unlist(st_is_within_distance(flow_seed, flow_lines_to_draw_from, dist = 50)),]
  ...
  nearest_flow_markers = nearest_flow_markers[nearest_flow_markers$flow_id %in% flow_marker_ids_to_retain,]
  ...
  # Drop any markers that only touch the flow_seed.
  flow_marker_ids_to_retain = nearest_flow_markers$flow_id |> 
    map( ~ {
      
      flow_to_test = nearest_flow_markers[nearest_flow_markers$flow_id == .x,]
      
      # flow_direction_points = flow_to_test |> 
      #   st_cast("POINT", warn = F)
      
      non_flow_overlap_test = flow_to_test |> 
        st_join(
          bind_rows(flow_seed,
                    wbs_after_difference |> 
                      mutate(name = 'not_flow_seed')),
          st_intersects) |> 
        filter(!is.na(name)) |> 
        filter(name != 'flow_seed')
      
      if(nrow(non_flow_overlap_test) > 0){
        return(flow_to_test)
      }
    }) |> 
    bind_rows() |> 
  ...
  for(id in nearest_flow_markers$flow_id){
    ...
    ggplot() + 
      geom_sf(data = flow_seed, fill = 'darkblue') +
      geom_sf(data = wbs_after_difference, fill = 'lightblue') + 
      geom_sf(color = 'red',data = nearest_flow_markers[nearest_flow_markers$flow_id == id,])
